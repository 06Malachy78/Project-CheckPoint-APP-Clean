...
export default function GlobalActivityFeed({
  ...
  const sortedItems = useMemo(() => {
    const baseItems = sortBy === 'friends' ? friendsFeedItems : feedItems;
    const filteredItems = sortBy === 'friends'
      ? baseItems.filter((item) => item?.user_id && followingSet.has(item.user_id))
      : baseItems;
  ...
  }, [feedItems, followingSet, friendsFeedItems, sortBy]);

  const friendsEmptyMessage = useMemo(() => {
    if (!isFriendsMode || sortedItems.length > 0) {
      return '';
    }
    ...
    return "People you follow haven't posted any checkpoints yet.";
  }, [followingUserIds, isFriendsMode, sortedItems.length]);
  ...
  return (
    <div className="space-y-8">
      ...
      {isFriendsMode && sortedItems.length === 0 ? (
